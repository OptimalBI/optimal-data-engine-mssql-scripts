USE [ode_config]
GO
/****** Object:  StoredProcedure [dbo].[ODE_link_sat_config]    Script Date: 8/02/2017 10:12:42 a.m. ******/
SET ANSI_NULLS ON
GO
...
,@StageTable VARCHAR(128)			= 'Link_Lookup_CustomerType'
...
,@LinkName VARCHAR(128)				= 'Lookup_CustomerType'
...
,@SatelliteName VARCHAR(128)		= 'Link_Lookup_CustomerType'
...
DECLARE @Hub_Key_List TABLE (link_key_name varchar(128), hub_name varchar(128), hub_column_name varchar(128), column_name varchar(128), OrdinalPosition INT IDENTITY(1,1))

INSERT @Hub_Key_List  VALUES ('Customer','Customer', 'Customer_KeyType', 'Customer_KeyType')
INSERT @Hub_Key_List  VALUES ('Customer','Customer', 'CustomerID', 'CustomerID')
INSERT @Hub_Key_List  VALUES ('Sales_Lookups','Sales_Lookups', 'LookupsID', 'LookupsID')
...
-- Create the Release:
EXECUTE @release_key = [dv_release].[dv_release_master_insert] 
...
EXECUTE @link_key = [dbo].[dv_link_insert] 
			 @link_name = @LinkName
...
select * from dbo.dv_column where table_key = @source_table_key
...
BEGIN   
	  ...
	  if @SatelliteOnly = 'N'
			begin
			EXECUTE  @link_key_column_key = [dbo].[dv_link_key_insert] 
											 @link_key				= @link_key
	  ...
	  WHILE @thisHub = @curLinkHub_hub_name
	    ...
	    and @@FETCH_STATUS = 0 
	  BEGIN	
	        select 'bb', @StageTable, @curLinkHub_column_name, * from [dbo].[dv_column] c
